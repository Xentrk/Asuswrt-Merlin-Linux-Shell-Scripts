...
# Description:
#  Helpful utility to
#  1) Save nvram dhcp_staticlist and dhcp_hostnames to /jffs/. This will allow you to restore the values after performing a factory reset.
#  2) Restore nvram dhcp_staticlist and dhcp_hostnames from /jffs/.
...
#  7) Backup nvram dhcp_staticlist and dhcp_hostnames to /jffs/ and clear nvram values.
...
Menu_DHCP_Staticlist() {
  ...
  while true; do
    ...
    printf '%b[1]%b - Save nvram dhcp_staticlist and dhcp_hostnames to /jffs/\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
    printf '%b[2]%b - Restore nvram dhcp_staticlist and dhcp_hostnames from /jffs/\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
    ...
    printf '%b[7]%b - Backup nvram dhcp_staticlist and dhcp_hostnames to /jffs/ and clear nvram values\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
...
Restore_DHCP_Staticlist_nvram() {
  nvram set dhcp_staticlist="$(cat /jffs/dhcp_staticlist.txt)"
  nvram commit
  sleep 1
...
Restore_DHCP_Hostnames() {
  if [ "$MODEL" = "RT-AC86U" ] || [ "$MODEL" = "RT-AX88U" ]; then #HND Routers store hostnames in a file
    ...
    nvram set dhcp_hostnames="$(cat /jffs/dhcp_hostnames.txt)"
    nvram commit
    sleep 1
