...
Menu_DHCP_Staticlist() {
  ...
  while true; do
    ...
    printf '%b[3]%b - Preview DHCP Static List in dnsmasq.conf.add format\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
    printf '%b[4]%b - Append Output DHCP Static List to dnsmasq.conf.add & Disable Manual Assignment\n' "${COLOR_GREEN}" "${COLOR_WHITE}"
